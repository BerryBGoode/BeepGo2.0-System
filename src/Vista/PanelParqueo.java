...
public class PanelParqueo extends javax.swing.JPanel {
    ...
    public PanelParqueo() {
        initComponents();
        String[] headerpark = {"IDDetalleAcceso", "Estacionamiento", "Fecha", "IDAcceso", "Hora", "IDParqueo", "Carnet", "IDVehiculo", "Nombres", "Apellidos", "Placa", "Ubicación", "Parqueo", "Modificar", "Eliminar"};
        ...
        TbParqueosWhite.removeColumn(TbParqueosWhite.getColumnModel().getColumn(7));
            ...
            if (row < TbParqueosWhite.getRowCount() && row >= 0 && col < TbParqueosWhite.getColumnCount() && col >= 0) {
                ...
                if (obj instanceof UWPButton) {
                    ...
                    if (btn.getName().equals("btnModificar")) {
                        ...
                        park.setStation(Integer.valueOf(tb.getModel().getValueAt(tb.getSelectedRow(), 1).toString()));
                        park.setPark(tb.getModel().getValueAt(tb.getSelectedRow(), 12).toString());
                        park.setVisible(true);
    ...
    public void getdataPark() {
        String tablename = "vwEstacionamientos";
        if (tbpark.getRowCount() > 0) {//si hay columnas antes de cargar 
            ...
            try {
                ...
                while (rs.next()) {
                    Object[] data = {rs.getInt("idEstacionamiento"), rs.getInt("numero_estacionamiento"), rs.getDate("fecha"), rs.getInt("idAcceso"), rs.getTime("hora"), rs.getInt("idParqueo"), rs.getString("Carnet"), rs.getInt("idVehiculo"), rs.getString("nombre_p"), rs.getString("apellido_p"), rs.getString("placa"), rs.getString("Ubicacion"), rs.getInt("numero_parqueo"), btnupdate, btndelete};
                    tbpark.addRow(data);
